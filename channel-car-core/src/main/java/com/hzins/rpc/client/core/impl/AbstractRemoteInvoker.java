...
import com.hzins.common.ObjectMapperFactory;
import com.hzins.common.ObjectMapperHelper;
...
public abstract class AbstractRemoteInvoker implements Invoker {
    ...
    protected final static Logger logger=LoggerFactory.getLogger(AbstractRemoteInvoker.class);
    
    protected final static ObjectMapper objectMapper = ObjectMapperFactory.getObjectMapper();

    protected JavaType getResponseType(Type type) {
	return objectMapper.getTypeFactory().constructType(type);
    }

    public abstract <T> T invoke(Type returnType, Object api, Object param,ServiceInfo serverInfo);
	...
	String base =Configure.Common.baseUrl;//FIXME 从配置文件获取
	...
	logger.info(ObjectMapperHelper.toJson(objectMapper,param));
	if (RpcType.HTTP.equals(rpcType)) {
	    return invoke(method.getGenericReturnType(), base, param,serverInfo);
	}else{
